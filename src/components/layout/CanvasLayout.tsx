...
export default function CanvasLayout({
  ...
  // 因此用户不会看到旧的错误高度。
  // 另设单次延时重测，处理图表等异步渲染的内容（recharts 等通常在 mount 后才完成布局）。
  //
  // 【抖动修复】原来使用 3 个延时（200/600/1500ms）分别触发 measureAndAdjust，
  // 每次都可能改变 h → 触发压缩 → 触发 onLayoutChange → 再触发测量 → 抖动。
  // 改为单次 300ms 延时，配合 handleLayoutChange 中忽略 y 的修复，彻底消除循环。
  useLayoutEffect(() => {
    if (!canRender) return;
    ...
    // 单次延时重测：处理 recharts 等异步渲染内容
    const timeout = window.setTimeout(() => measureAndAdjust(), 300);
